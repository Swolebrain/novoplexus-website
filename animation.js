(function () {
    ...
    function distance(p1, p2) {
        ...
        if (minDist >= dist) {
            ...
            // var ax = dx / (1e5 * velocity), ay = dy / (1e5 * velocity);
            // p1.vx -= ax, p1.vy -= ay, p2.vx += ax, p2.vy += ay;
            // if (p1.vx < -1.5) p1.vx = -1.5;
            // if (p1.vx > 1.5) p1.vx = 1.5;
            // if (p1.vy < -1.5) p1.vy = -1.5;
            // if (p1.vy > 1.5) p1.vy = 1.5;
            // if (p2.vx < -1.5) p2.vx = -1.5;
            // if (p2.vx > 1.5) p2.vx = 1.5;
            // if (p2.vy < -1.5) p2.vy = -1.5;
            // if (p2.vy > 1.5) p2.vy = 1.5;
        }
    }
    ...
    function init() {
        ...
        ctx = document.getElementById('space').getContext("2d"),
            particleCount = window.innerWidth/25, particles = [], particleRadius = Math.min(3,window.innerWidth/50),
            minDist = 200, velocity = 1, colors = [
            {r: 237, g: 42, b: 128},
            {r: 193, g: 36, b: 47},
            {r: 241, g: 89, b: 43}
        ];
